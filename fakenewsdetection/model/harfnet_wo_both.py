"""
HARFNET-V3 — Ablation: w/o BilinearInteraction & w/o OverAlignModule
BilinearInteraction + OverAlignModule 모두 제거한 버전.
head4 입력: [F_sty, F_ver]  (d*2)
...
LOG_TAG   = "HARFNET-VER3-woBilinear-woOA"
LOG_BRAND = "HARFNET-VER3-woBilinear-woOA"
...
# 10. 메인 모델
# ─────────────────────────────────────────────────────────────
class HARFNETver3_woBilinear_woOA(nn.Module):
    def __init__(self, roberta_name=ROBERTA, clip_name=CLIP_RN101,
        ...
        # head4: d*2 (F_sty + F_ver)
        self.head4 = nn.Sequential(
            nn.Linear(d * 2, 512), nn.LayerNorm(512),
    ...
    def forward(self, input_ids, attention_mask, pixel_values,
        ...
        # BilinearInteraction 없이 F_sty + F_ver만 concatenation
        logits4 = torch.nan_to_num(
            self.head4(torch.cat([F_sty, F_ver], dim=-1)),
...
def total_loss(out, y, has_image, args):
    """
    OA_Loss와 AF_Cond_BCE는 OverAlignModule 제거로 미사용.
    활성 손실: Focal + BCE(Fake) + BCE(Auth) + SupCon
    """
...
def _monitor(model, loader, device) -> str:
    """OverAlignModule 제거로 모니터링 항목을 p_fake/p_auth/global_sim으로 축소."""
    model.eval()
    try:
        with torch.no_grad():
...
# 13. 학습 루프
# ─────────────────────────────────────────────────────────────
def freeze_backbones(model: HARFNETver3_woBilinear_woOA):
...
def _new_model(args, device):
    return HARFNETver3_woBilinear_woOA(
...
def run_single(csv, root, device, tok, prep, args, freeze, preamble=""):
    ...
    save_checkpoint(os.path.join(
        args.checkpoint_dir,
        f"harfnet_ver3_wobilinear_wooa_single_{ts2}.pt"), model, args, m)
    summ = (f"{LOG_TAG}: 4-way F1={m['f1_4']:.4f} "
            f"H/A={m['f1_ha']:.4f} R/F={m['f1_rf']:.4f}")
    print("\n"+"="*70+"\n"+summ+"\n"+"="*70)
    path = os.path.join(RESULT_DIR, f"harfnet_ver3_wobilinear_wooa_single_{ts}.txt")
...
def run_kfold(csv, root, device, tok, prep, args, freeze, preamble=""):
    ...
    for fold, (str_, sva_) in enumerate(skf.split(np.zeros(len(tva)), y[tva]), 1):
        ...
        save_checkpoint(os.path.join(
            args.checkpoint_dir,
            f"harfnet_ver3_wobilinear_wooa_kfold_{ts2}_fold{fold}.pt"),
            model, args, m)
        summ = (f"{LOG_TAG}: 4-way F1={m['f1_4']:.4f} "
                f"H/A={m['f1_ha']:.4f} R/F={m['f1_rf']:.4f}")
        print(f"\n[Fold {fold}] {summ}")
    ...
    all_rep.write(
        f"VER3-woBilinear-woOA\t{_m('f1_ha'):.4f}\t{_m('f1_rf'):.4f}\t{_m('f1_4'):.4f}\n")
    path = os.path.join(RESULT_DIR, f"harfnet_ver3_wobilinear_wooa_kfold_{ts}.txt")
...
# ─────────────────────────────────────────────────────────────
def main():
    pa = argparse.ArgumentParser(description="HARFNET-VER3-woBilinear-woOA")
    ...
    # OA/AF 관련 lambda는 이 버전에서 미사용이나 파싱은 유지 (호환성)
