...
HARFNET-VER3-WO-V
==================
Ablation: VeracityModule 제거
- VeracityModule (self.vm) 제거
- BilinearInteraction -> 2-way (auth x oa) 로 교체
- head4 입력: [F_sty, F_oa, F_bi] (d*3)
- 제거 손실: fake BCE, SupCon
"""
...
LOG_TAG = "HARFNET-VER3-WO-V"
LOG_BRAND = "HARFNET-VER3-WO-V"
...
class HARFNETver3_WO_V(nn.Module):
    ...
    def forward(self, input_ids, attention_mask, pixel_values, has_image, clip_text_tokens=None):
        ...
        global_sim = (t_n * v_n).sum(-1).clamp(-1.0, 1.0) * has_image
        ...
        logits4 = torch.nan_to_num(
            self.head4(torch.cat([f_sty, f_oa, f_bi], dim=-1)),
            nan=0.0,
            posinf=20.0,
            neginf=-20.0,
        )
...
def total_loss_wo_v(out, y, has_image, args):
    ...
    loss += args.lambda_auth_bce * base.attribute_bce_loss(out["p_auth"], (y >= 2), has_image)
...
def run_epoch(model, loader, device, optimizer, train, epoch_idx=None, args=None):
    ...
    with ctx:
        for batch in it:
            ...
            loss = total_loss_wo_v(out, y, hi, args)
...
def _monitor(model, loader, device) -> str:
    ...
    try:
        with torch.no_grad():
            bufs = {k: [] for k in ["lbl", "hi", "pa", "paf", "oam", "oau", "gs"]}
...
def _new_model(args, device):
    return HARFNETver3_WO_V(roberta_name=args.roberta, clip_name=args.clip_model).to(device)
...
def run_single(csv, root, device, tok, prep, args, freeze, preamble=""):
    ...
    base.save_checkpoint(os.path.join(args.checkpoint_dir, f"harfnet_ver3_wo_v_single_{ts2}.pt"), model, args, m)
    path = os.path.join(base.RESULT_DIR, f"harfnet_ver3_wo_v_single_{ts}.txt")
...
def run_kfold(csv, root, device, tok, prep, args, freeze, preamble=""):
    ...
    for fold, (str_, sva_) in enumerate(skf.split(np.zeros(len(tva)), y[tva]), 1):
        ...
        base.save_checkpoint(
            os.path.join(args.checkpoint_dir, f"harfnet_ver3_wo_v_kfold_{ts2}_fold{fold}.pt"),
            model,
            args,
            m,
        )
    ...
    all_rep.write(f"WO-V\t{_m('f1_ha'):.4f}\t{_m('f1_rf'):.4f}\t{_m('f1_4'):.4f}\n")
    path = os.path.join(base.RESULT_DIR, f"harfnet_ver3_wo_v_kfold_{ts}.txt")
    with open(path, "w", encoding="utf-8") as f:
...
def main():
    pa = argparse.ArgumentParser(description="HARFNET-VER3 without VeracityModule")
    ...
    pa.add_argument("--lambda_fake_bce", type=float, default=0.0)  # 미사용
    ...
    pa.add_argument("--lambda_con", type=float, default=0.0)  # 미사용
