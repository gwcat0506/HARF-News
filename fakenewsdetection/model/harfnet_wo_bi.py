"""
HARFNET-V3 — Ablation: w/o BilinearInteraction
BilinearInteraction 모듈을 완전히 제거한 버전.
head4 입력: [F_sty, F_ver, F_oa]  (d*3)
"""
...
# BilinearInteraction 제거로 BILINEAR_RANK 미사용
PROJ_DIM      = 128
TEMPERATURE   = 0.07
...
LOG_TAG   = "HARFNET-VER3-woBilinear"
LOG_BRAND = "HARFNET-VER3-woBilinear"
...
# ─────────────────────────────────────────────────────────────
# BilinearInteraction 클래스 전체 삭제.
# head4 입력 차원: d*3 (F_sty + F_ver + F_oa)


# ─────────────────────────────────────────────────────────────
# 10. 메인 모델
# ─────────────────────────────────────────────────────────────
class HARFNETver3_woBilinear(nn.Module):
    ...
    def forward(self, input_ids, attention_mask, pixel_values,
        ...
        # BilinearInteraction 없이 단순 concatenation
        logits4 = torch.nan_to_num(
            self.head4(torch.cat([F_sty, F_ver, F_oa], dim=-1)),
...
# 13. 학습 루프
# ─────────────────────────────────────────────────────────────
def freeze_backbones(model: HARFNETver3_woBilinear):
...
def run_single(csv, root, device, tok, prep, args, freeze, preamble=""):
    ...
    save_checkpoint(os.path.join(
        args.checkpoint_dir, f"harfnet_ver3_wobilinear_single_{ts2}.pt"), model, args, m)
    summ = (f"{LOG_TAG}: 4-way F1={m['f1_4']:.4f} "
            f"H/A={m['f1_ha']:.4f} R/F={m['f1_rf']:.4f}")
    print("\n"+"="*70+"\n"+summ+"\n"+"="*70)
    path = os.path.join(RESULT_DIR, f"harfnet_ver3_wobilinear_single_{ts}.txt")
...
def run_kfold(csv, root, device, tok, prep, args, freeze, preamble=""):
    ...
    for fold, (str_, sva_) in enumerate(skf.split(np.zeros(len(tva)), y[tva]), 1):
        ...
        save_checkpoint(os.path.join(
            args.checkpoint_dir,
            f"harfnet_ver3_wobilinear_kfold_{ts2}_fold{fold}.pt"),
            model, args, m)
        summ = (f"{LOG_TAG}: 4-way F1={m['f1_4']:.4f} "
                f"H/A={m['f1_ha']:.4f} R/F={m['f1_rf']:.4f}")
        print(f"\n[Fold {fold}] {summ}")
    ...
    all_rep.write(f"VER3-woBilinear\t{_m('f1_ha'):.4f}\t{_m('f1_rf'):.4f}\t{_m('f1_4'):.4f}\n")
    path = os.path.join(RESULT_DIR, f"harfnet_ver3_wobilinear_kfold_{ts}.txt")
...
# ─────────────────────────────────────────────────────────────
def main():
    pa = argparse.ArgumentParser(description="HARFNET-VER3-woBilinear")
