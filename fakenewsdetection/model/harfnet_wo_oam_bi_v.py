"""
HARFNET-V3 — Ablation: w/o BilinearInteraction & w/o OverAlignModule & w/o VeracityModule
BilinearInteraction + OverAlignModule + VeracityModule 모두 제거한 버전.
head4 입력: [F_sty]  (d)
제거된 손실항: lambda_oa·OA_Loss, lambda_af_bce·AF_Cond_BCE,
              lambda_fake_bce·BCE(Fake), lambda_con·SupCon
"""
...
# LAMBDA_FAKE_BCE  미사용 (VeracityModule 제거)
# LAMBDA_OA        미사용 (OverAlignModule 제거)
LAMBDA_AUTH_BCE  = 0.15
# LAMBDA_CON       미사용 (VeracityModule 제거 → z_ver 없음)
...
LOG_TAG   = "HARFNET-VER3-woBilinear-woOA-woVM"
LOG_BRAND = "HARFNET-VER3-woBilinear-woOA-woVM"
...
# ─────────────────────────────────────────────────────────────
# 7. VeracityModule  → 제거됨
# 8. OverAlignModule → 제거됨
# 9. BilinearInteraction → 제거됨
# head4 입력 차원: d (F_sty만 사용)
# ─────────────────────────────────────────────────────────────
...
# 10. 메인 모델
# ─────────────────────────────────────────────────────────────
class HARFNETver3_woBilinear_woOA_woVM(nn.Module):
    def __init__(self, roberta_name=ROBERTA, clip_name=CLIP_RN101,
        ...
        # CLIP은 이미지 전처리용으로만 로드 (VeracityModule 제거로 시각 특징 불필요)
    ...
    def forward(self, input_ids, attention_mask, pixel_values,
        ...
        # VeracityModule 호출 없음 (시각 특징 추출 불필요)
        ...
        return {
            ...
            "p_auth":  p_auth,
            "z_auth":  z_auth,
            # p_fake / z_ver / global_sim / p_af / oa_* 없음
        }
...
def total_loss(out, y, has_image, args):
    """
    VeracityModule 제거로 BCE(Fake)·SupCon 비활성.
    OA_Loss·AF_Cond_BCE도 OverAlignModule 제거로 비활성.
    활성 손실: Focal + BCE(Auth)
    """
    loss = focal_loss(out["logits4"], y)

    # BCE(Fake) 제거 — p_fake 없음 (VeracityModule 제거)
    # SupCon 제거   — z_ver 없음 (VeracityModule 제거)
...
def _monitor(model, loader, device) -> str:
    """VeracityModule 제거로 모니터링 항목을 p_auth만으로 축소."""
    model.eval()
    try:
        with torch.no_grad():
            bufs = {k: [] for k in ["lbl","hi","pa"]}
            ...
            pa  = torch.cat(bufs["pa"])

            parts = []
            for ci, cn in enumerate(CLASS_NAMES):
                mask = (lbl == ci) & (hi > 0.5)
                if mask.sum() > 0:
                    parts.append(f"{cn}:pa={pa[mask].mean():.2f}")
...
# 13. 학습 루프
# ─────────────────────────────────────────────────────────────
def freeze_backbones(model: HARFNETver3_woBilinear_woOA_woVM):
...
def _new_model(args, device):
    return HARFNETver3_woBilinear_woOA_woVM(
...
def run_single(csv, root, device, tok, prep, args, freeze, preamble=""):
    ...
    save_checkpoint(os.path.join(
        args.checkpoint_dir,
        f"harfnet_ver3_wobilinear_wooa_wovm_single_{ts2}.pt"), model, args, m)
    summ = (f"{LOG_TAG}: 4-way F1={m['f1_4']:.4f} "
            f"H/A={m['f1_ha']:.4f} R/F={m['f1_rf']:.4f}")
    print("\n"+"="*70+"\n"+summ+"\n"+"="*70)
    path = os.path.join(RESULT_DIR, f"harfnet_ver3_wobilinear_wooa_wovm_single_{ts}.txt")
...
def run_kfold(csv, root, device, tok, prep, args, freeze, preamble=""):
    ...
    for fold, (str_, sva_) in enumerate(skf.split(np.zeros(len(tva)), y[tva]), 1):
        ...
        save_checkpoint(os.path.join(
            args.checkpoint_dir,
            f"harfnet_ver3_wobilinear_wooa_wovm_kfold_{ts2}_fold{fold}.pt"),
            model, args, m)
        summ = (f"{LOG_TAG}: 4-way F1={m['f1_4']:.4f} "
                f"H/A={m['f1_ha']:.4f} R/F={m['f1_rf']:.4f}")
        print(f"\n[Fold {fold}] {summ}")
    ...
    all_rep.write(
        f"VER3-woBilinear-woOA-woVM\t{_m('f1_ha'):.4f}\t{_m('f1_rf'):.4f}\t{_m('f1_4'):.4f}\n")
    path = os.path.join(RESULT_DIR, f"harfnet_ver3_wobilinear_wooa_wovm_kfold_{ts}.txt")
...
# ─────────────────────────────────────────────────────────────
def main():
    pa = argparse.ArgumentParser(description="HARFNET-VER3-woBilinear-woOA-woVM")
    ...
    pa.add_argument("--lambda_fake_bce", type=float, default=0.0)   # 미사용 (VM 제거)
    pa.add_argument("--lambda_oa",       type=float, default=0.0)   # 미사용
    pa.add_argument("--lambda_auth_bce", type=float, default=LAMBDA_AUTH_BCE)
    pa.add_argument("--lambda_con",      type=float, default=0.0)   # 미사용 (VM 제거)
    ...
    print(f"[손실] Focal"
          ...
          f"  [BCE(Fake)/SupCon/OA_Loss/AF_BCE 비활성]\n")
