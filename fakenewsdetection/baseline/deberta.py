"""
DeBERTa-v3 text-only baseline (4-class).
...
LR = 2e-5
NUM_WORKERS = 4
# hier2는 AMP·gradient accumulation 미사용
USE_AMP = False
ACCUM_STEPS = 1
...
# 2. 데이터 로드
# -------------------------
print("Loading HARFM.csv for DeBERTa-v3 text-only baseline...")
...
# 멀티모달 유효 샘플만 유지한 뒤, 텍스트만 사용
data["image_path"] = data["image_path"].apply(lambda p: _resolve_image_path(p, _FND_ROOT))
n_before = len(data)
data = data[
...
print(f"[전처리] 멀티모달 필터(image_path 유효): {n_before} → {len(data)}")
...
# -------------------------
# 5. DeBERTa-v3 Text-only 4-class 모델
# -------------------------
class DebertaV3TextOnly4C(nn.Module):
    """
    DeBERTa-v3-base + 4-class head
    """
...
# -------------------------
def evaluate_4class_deberta(model, loader):
    model.eval()
    ...
    with torch.no_grad():
        for batch in tqdm(loader, desc="Evaluation (DeBERTa-v3 text-only 4C)"):
    ...
    print("\n=== DeBERTa-v3 Text-only Human vs AI report ===")
    ...
    print("\n=== DeBERTa-v3 Text-only Real vs Fake report ===")
    print(classification_report(
        all_true_rf, all_pred_rf,
    ...
    print("\n=== DeBERTa-v3 Text-only 4-class report ===")
    print(classification_report(
        all_true_4, all_pred_4,
...
# -------------------------
def _validate_deberta_four_class(model, val_loader):
    model.eval()
    all_true, all_pred = [], []
    with torch.no_grad():
...
# -------------------------
# 9. DeBERTa-v3 4-class 학습 함수
# -------------------------
def train_deberta_4c(model, train_loader, val_loader, epochs=EPOCHS):
    model.to(DEVICE)
    ...
    for epoch in range(epochs):
        ...
        for batch in tqdm(train_loader, desc=f"[DeBERTa-4C] Epoch {epoch+1}"):
            ids  = batch["input_ids"].to(DEVICE, non_blocking=True)
            attn = batch["attention_mask"].to(DEVICE, non_blocking=True)
            lab4 = batch["label4"].to(DEVICE, non_blocking=True)
        ...
        print(f"[DeBERTa-4C] avg_train_loss={avg_train_loss:.4f}")

        # Validation
        val_f1 = _validate_deberta_four_class(model, val_loader)
        print(f"[DeBERTa-4C] val_macro_f1(4-way)={val_f1:.4f}")

        if val_f1 > best_val:
            best_val   = val_f1
            best_state = copy.deepcopy(model.state_dict())
            print("[DeBERTa-4C] Saved best model state (in-memory).")

        if early.step(val_f1):
            print(
                f"[DeBERTa-4C] [EarlyStop] No improvement for {EARLY_STOP_PATIENCE} epochs "
...
# 11. K-Fold Runner
# -------------------------
def kfold_deberta_4c_with_reports(
    full_df,
    k=KFOLD_SPLITS,
    ...
    print(f"[KFold-DeBERTa-4C] Split: TrainVal={len(trainval_df)}, Test={len(test_df)}")
    ...
    all_report.write(f"###### DeBERTa-v3 Text-only 4C (k={k}) ######\n")
    ...
    for fold, (tr_idx, va_idx) in enumerate(
        ...
        with redirect_stdout(tee):
            print("\n" + "=" * 70)
            print(f"[Fold {fold}] DeBERTa-v3 Text-only 4-class")
            print("=" * 70)
            model = DebertaV3TextOnly4C().to(DEVICE)
            model = train_deberta_4c(model, train_loader, val_loader)
            res = evaluate_4class_deberta(model, test_loader)

            print("\n" + "=" * 70)
            print(f"[Fold {fold}] SUMMARY (DeBERTa-v3 Text-only, Acc/F1 on Test)")
            print("=" * 70)
            print(
                f"DeBERTa-4C: "
    ...
    all_report.write("\n\n" + "#"*90 + "\n")
    all_report.write(
        "### GLOBAL SUMMARY (DeBERTa-v3 text-only, Mean over {} folds)\n".format(k)
    )
    all_report.write("#"*90 + "\n")
    all_report.write(
        f"DeBERTa-4C (text-only): "
    ...
    all_report.write("\n[TABLE_SUMMARY]\n")
    all_report.write("Variant\tHA_Acc\tHA_F1\tRF_Acc\tRF_F1\t4C_Acc\t4C_F1\n")
    all_report.write(
        f"DeBERTa-v3-4C(text-only)\t"
    ...
    output_path = os.path.join(
        RESULT_DIR, f"kfold_deberta_textonly_4c_{timestamp}.txt"
    )
...
# 12. 단일 split
# -------------------------
def get_single_split_loaders_deberta(full_df):
...
if __name__ == "__main__":
    ...
    print_memory_usage()
    print(
        f"DeBERTa-v3 text-only Baseline | "
        ...
        f"EPOCHS={EPOCHS}, MAX_LEN={MAX_LEN}, SEED={SEED}"
    )
    print(f"Dataset: HARFM.csv (final_headline, 4_way_label) - text-only\n")

    if USE_KFOLD:
        kfold_deberta_4c_with_reports(
            data,
            k=KFOLD_SPLITS,
            test_size=KFOLD_TEST_SIZE,
            shuffle=KFOLD_SHUFFLE,
        ...
        train_loader, val_loader, test_loader = get_single_split_loaders_deberta(data)
        model = DebertaV3TextOnly4C().to(DEVICE)
        model = train_deberta_4c(model, train_loader, val_loader)
        _ = evaluate_4class_deberta(model, test_loader)
