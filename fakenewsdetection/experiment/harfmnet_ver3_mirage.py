...
# 10. 메인 모델
# ─────────────────────────────────────────────────────────────
class HARFMNETver3_Binary(nn.Module):
    """이진 분류용 HARFMNET (head4 → head2)."""
    ...
    @classmethod
    def from_harfm_checkpoint(cls, ckpt_path, device,
...
def run_official_split(args):
    ...
    if args.harfm_ckpt:
        model=HARFMNETver3_Binary.from_harfm_checkpoint(
            args.harfm_ckpt,DEVICE,args.roberta,args.clip_model)
    else:
        model=HARFMNETver3_Binary(args.roberta,args.clip_model).to(DEVICE)
    ...
    save_checkpoint(
        os.path.join(CHECKPOINT_DIR, f"harfmnet_mirage_{ts}.pt"),
        model, args, all_metrics)

    path=os.path.join(RESULT_DIR,f"harfmnet_mirage_{ts}.txt")
...
def run_kfold_split(args):
    ...
    kfold_ckpt_path = os.path.join(
        CHECKPOINT_DIR, f"harfmnet_mirage_kfold{args.kfold_splits}_{ts}.pt"
    )
    torch.save(
        {
            "config": vars(args),
    ...
    path = os.path.join(RESULT_DIR, f"harfmnet_mirage_kfold{args.kfold_splits}_{ts}.txt")
...
def main():
    ...
    print(f"       from_harfm_checkpoint 필터 'head4.'")
