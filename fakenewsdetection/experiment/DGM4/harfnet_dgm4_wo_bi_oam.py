"""
HARFNET-VER3 × DGM4  —  Ablation: w/o BI+OAM (Bilinear + OverAlign Module)
=============================================================================
"""
...
LOG_TAG   = "HARFNET-DGM4-woBIOAM"
LOG_BRAND = "HARFNET-DGM4-woBIOAM"
...
# ─────────────────────────────────────────────────────────────
# 5. 서브모듈
# [ABLATION] OverAlignModule + BilinearInteraction 완전 제거
# ─────────────────────────────────────────────────────────────
...
# [ABLATION] BilinearInteraction 클래스 삭제됨


# ─────────────────────────────────────────────────────────────
# 6. 메인 모델 — w/o BI + OAM
# ─────────────────────────────────────────────────────────────
class HARFNETdgm4_woBIOAM(nn.Module):
    """
    [ABLATION] OverAlignModule + BilinearInteraction 동시 제거 버전.
    F_oa / F_bi 를 zero-tensor 로 대체.
    """
    ...
    def forward(self, input_ids, attention_mask,
        ...
        # oa_aux 필드를 dummy로 채워 collect_predictions에서 key error 방지
        ...
        for batch in _tqdm(loader, desc=f"[{LOG_TAG}] 예측 수집"):
            ...
            for i in range(bin_lbl.size(0)):
                rows.append({
                    ...
                    # [ABLATION] OAM 없으므로 dummy zeros 기록
                    "oa_mean":        round(float(out["oa_mean"][i]), 4),
                    "oa_std":         round(float(out["oa_std"][i]), 4),
                    "oa_uniformity":  round(float(out["oa_uniformity"][i]), 4),
...
def run_official(dfs, device, tok, prep, args, configure, result_dir):
    ...
    model = HARFNETdgm4_woBIOAM(args.roberta, args.clip_model).to(device)
    if configure:
    ...
    if args.case_study_n > 0:
        ...
        save_case_study_report(cases, full_df, m, "woBIOAM_official", result_dir)

    ckpt_path = os.path.join(args.checkpoint_dir, f"harfnet_dgm4_woBIOAM_{ts}.pt")
    save_checkpoint(ckpt_path, model, args, m)
    summ = (f"{LOG_TAG}: BinF1={m['bin_f1']:.4f}  AUC={m.get('bin_auc', float('nan')):.4f}")
    print("\n" + "="*70 + "\n" + summ + "\n" + "="*70)
    path = os.path.join(result_dir, f"harfnet_dgm4_woBIOAM_{ts}.txt")
...
def run_kfold(dfs, device, tok, prep, args, configure, result_dir):
    ...
    for fold, (tr_idx, va_idx) in enumerate(skf.split(np.zeros(len(df_tv)), y_tv), 1):
        ...
        ckpt = os.path.join(args.checkpoint_dir,
                            f"harfnet_dgm4_woBIOAM_kfold_{ts}_fold{fold}.pt")
    ...
    all_rep.write(f"\n\n{'#'*80}\n### SUMMARY\n{g}\n")
    path = os.path.join(result_dir, f"harfnet_dgm4_woBIOAM_kfold_{ts}.txt")
    with open(path, "w", encoding="utf-8") as f:
...
def main():
    pa = argparse.ArgumentParser(description="HARFNET-VER3 × DGM4 — Ablation: w/o BI+OAM")
