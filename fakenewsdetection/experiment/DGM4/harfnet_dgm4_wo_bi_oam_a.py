"""
HARFNET-VER3 × DGM4  —  Ablation: w/o BI + OAM + AM
=======================================================
"""
...
LOG_TAG   = "HARFNET-DGM4-woBIOAMA"
LOG_BRAND = "HARFNET-DGM4-woBIOAMA"
...
# ─────────────────────────────────────────────────────────────
# 6. 메인 모델 — w/o BI + OAM + AM  (VM Only)
#
#  ┌────────────────────────────────────────────────────────┐
#  │ 원본    F_sty + F_ver + F_oa + F_bi  →  d×4 → head    │
#  │ 본파일  zeros + F_ver + zeros + zeros →  d×4 → head   │
#  └────────────────────────────────────────────────────────┘
# ─────────────────────────────────────────────────────────────
class HARFNETdgm4_woBIOAMA(nn.Module):
    ...
    def forward(self, input_ids, attention_mask,
        ...
        # AM 없음 → F_sty / p_auth / z_auth = zeros
        F_sty  = torch.zeros(B, self._d, device=device)
        p_auth = torch.zeros(B, device=device)
        z_auth = torch.zeros(B, PROJ_DIM, device=device)
        ...
        return {
            ...
            "p_fine":        p_fine,
            "p_auth":        p_auth,          # zeros — 손실에서 미사용
            "p_fake":        p_fake,
            "z_auth":        z_auth,          # zeros — 손실에서 미사용
            "z_ver":         z_ver,
            "global_sim":    gsim,
...
#    ✗ oa_reg_loss      (OAM 없음)
#    ✗ supcon(z_auth)   (AM 없음 → z_auth = zeros, 의미 없음)
#    ✓ binary_focal_loss
#    ✓ supcon(z_ver)    (VM 유지)
# ─────────────────────────────────────────────────────────────
...
def total_loss(out, binary_label, fine_labels, has_image, args):
    """
    focal(binary) + λ_con·supcon(z_ver)
    — z_auth = zeros이므로 supcon(z_auth) 제거 (학습 노이즈 방지)
    """
    l_bin = binary_focal_loss(out["binary_logit"], binary_label)
    l_con = supcon_loss(out["z_ver"], binary_label)
        ...
        for batch in _tqdm(loader, desc=f"[{LOG_TAG}] 예측 수집"):
            ...
            for i in range(bin_lbl.size(0)):
                rows.append({
                    ...
                    "p_auth":         round(float(out["p_auth"][i]), 4),   # zeros
...
def run_official(dfs, device, tok, prep, args, configure, result_dir):
    ...
    rep.write(f"[제거] BI + OAM + AM  [유지] VM\n")
    rep.write(f"[head] d×4 → 256 → 1  (zeros + F_ver + zeros×2, 차원 원본 유지)\n")
    rep.write(f"[손실] Focal + λ_con·supcon(z_ver)\n\n")
    ...
    model = HARFNETdgm4_woBIOAMA(args.roberta, args.clip_model).to(device)
    if configure:
    ...
    if args.case_study_n > 0:
        ...
        save_case_study_report(cases, full_df, m, "woBIOAMA_official", result_dir)

    ckpt_path = os.path.join(args.checkpoint_dir, f"harfnet_dgm4_woBIOAMA_{ts}.pt")
    save_checkpoint(ckpt_path, model, args, m)
    summ = (f"{LOG_TAG}: BinF1={m['bin_f1']:.4f}  AUC={m.get('bin_auc', float('nan')):.4f}")
    print("\n" + "="*70 + "\n" + summ + "\n" + "="*70)
    path = os.path.join(result_dir, f"harfnet_dgm4_woBIOAMA_{ts}.txt")
...
def run_kfold(dfs, device, tok, prep, args, configure, result_dir):
    ...
    for fold, (tr_idx, va_idx) in enumerate(skf.split(np.zeros(len(df_tv)), y_tv), 1):
        ...
        ckpt = os.path.join(args.checkpoint_dir,
                            f"harfnet_dgm4_woBIOAMA_kfold_{ts}_fold{fold}.pt")
    ...
    all_rep.write(f"\n\n{'#'*80}\n### SUMMARY\n{g}\n")
    path = os.path.join(result_dir, f"harfnet_dgm4_woBIOAMA_kfold_{ts}.txt")
    with open(path, "w", encoding="utf-8") as f:
...
def main():
    pa = argparse.ArgumentParser(
        description="HARFNET-VER3 × DGM4 — Ablation: w/o BI+OAM+AM (VM Only)")
    ...
    print(f"[Ablation] w/o BI  w/o OAM  w/o AM  →  VM Only")
    print(f"[head]     d×4 → 256 → 1  (zeros + F_ver + zeros×2, 차원 원본 유지)")
    print(f"[손실]     Focal + {args.lambda_con}·supcon(z_ver)")
    print(f"           (supcon(z_auth) 제거 — z_auth=zeros 이므로)")
    print(f"[CLIP]     layer4/attnpool unfreeze (VM cross-attention 사용)")
