"""
HARFNET-VER3 × DGM4 벤치마크 실험 코드 
======================================================

사용 방법:
  python hier3_dgm4_fixed.py --data_root /path/to/dgm4_data

  # K-Fold 실행하려면:
  python hier3_dgm4_fixed.py --data_root /path/to/dgm4_data --kfold
...
BATCH                = 16
EPOCHS               = 10        
LR                   = 2e-4        # [FIX] 1e-4 → 2e-4 (new module 기준)
NUM_WORKERS          = 4
EARLY_STOP_PATIENCE  = 4         
EARLY_STOP_MIN_DELTA = 1e-4
WARMUP_EPOCHS        = 2           # [NEW] Linear warmup epoch 수
KFOLD_SPLITS         = 5
KFOLD_RANDOM_STATE   = 42
SEED                 = 42

# [FIX] 멀티태스크 손실 가중치 — 원래 의도대로 복원
LAMBDA_BINARY_BCE = 0.40
LAMBDA_FINE_BCE   = 0.30
LAMBDA_CON        = 0.10
LAMBDA_OA         = 0.20

# [NEW] 차등 LR 배율 (make_optimizer에서 사용)
LR_MULT_TEXT   = 0.05   # RoBERTa 상위 레이어
LR_MULT_VISUAL = 0.02   # CLIP layer4 + attnpool
LR_MULT_NEW    = 1.00   # 새 모듈 (from scratch)
...
CLIP_UNFREEZE_KEYS = ("layer4", "attnpool")   # [NEW] 부분 해동할 CLIP 레이어
...
LOG_TAG   = "HARFNET-DGM4"
LOG_BRAND = "HARFNET-DGM4"
...
# 6. 메인 모델
# ─────────────────────────────────────────────────────────────
class HARFNETdgm4(nn.Module):
    """
    DGM4 전용 HARFNET-VER3.
    [FIX] CLIP 시각 인코더 layer4 + attnpool 부분 해동
    """
    def __init__(self, roberta_name=ROBERTA,
        ...
        # [FIX] CLIP 기본 레이어 Freeze, layer4 + attnpool 만 Unfreeze
    ...
    def train(self, mode=True):
        super().train(mode)
        # CLIP은 항상 eval (BatchNorm 안정성 유지)
        self.clip_model.eval()
        return self
    ...
    def forward(self, input_ids, attention_mask,
        ...
        F_oa, p_fine, oa_aux = self.om(
            V_pat, T_cls, gsim, p_fake, p_auth, has_image)
...
# 7. 손실 함수
# ─────────────────────────────────────────────────────────────
# [FIX] pos_weight 2.0 → 1.0
# DGM4 train 셋은 real=fake=104,092 (완전 균형)이므로
# pos_weight=2.0은 fake를 과도하게 강조해 real 성능을 깎아먹음
...
# [FIX] 핵심 수정: 4개 손실항 모두 복원
# 원본 코드는 binary_focal_loss 하나만 사용하고 나머지 3개를 버렸음
# → AM/VM/OM 보조 모듈이 학습 신호를 거의 못 받아 BilinearInteraction에
#   랜덤 노이즈를 주입하는 효과가 나타남
def total_loss(out, binary_label, fine_labels, has_image, args):
    ...
    # z_auth(문체 일관성)와 z_ver(시각-텍스트 일치)를 각각 대조 학습
...
# 9. 옵티마이저 & 스케줄러 (신규)
# ─────────────────────────────────────────────────────────────
def make_optimizer(model: HARFNETdgm4, args) -> torch.optim.Optimizer:
    """
    [NEW] 3-Group 차등 학습률 옵티마이저
    ─ backbone_text   : RoBERTa 학습 가능 레이어 → lr * LR_MULT_TEXT
    ─ backbone_visual : CLIP layer4/attnpool     → lr * LR_MULT_VISUAL
    ─ new_modules     : AM/VM/OM/BI/헤드          → lr * LR_MULT_NEW
    """
...
def make_scheduler(optimizer: torch.optim.Optimizer, args):
    """
    [NEW] Linear Warmup (warmup_epochs) + Cosine Annealing
    """
...
def save_case_study_report(cases: dict, full_df: pd.DataFrame,
    ...
    buf.write("── [fake_cls별 신호 분포] ──\n")
    buf.write(cases["signal_summary"].to_string(index=False))
    buf.write("\n\n")

    buf.write("── [False Positive: Real→Fake 오분류] ──\n")
    if len(cases["fp_cases"]) == 0:
        buf.write("  없음\n")
    else:
        for _, row in cases["fp_cases"].head(5).iterrows():
            buf.write(f"  fake_cls={row['fake_cls']}"
                      f"  prob={row['binary_prob']:.3f}"
                      f"  p_auth={row['p_auth']:.3f}"
                      f"  oa_mean={row['oa_mean']:.3f}\n")
            txt = str(row["text"])
            buf.write(f"  텍스트: {txt[:100]}"
                      f"{'...' if len(txt) > 100 else ''}\n\n")

    buf.write("── [False Negative: Fake→Real 오분류] ──\n")
    if len(cases["fn_cases"]) == 0:
        buf.write("  없음\n")
    else:
        for _, row in cases["fn_cases"].head(5).iterrows():
            buf.write(f"  fake_cls={row['fake_cls']}"
                      f"  prob={row['binary_prob']:.3f}"
                      f"  p_fake={row['p_fake']:.3f}"
                      f"  oa_mean={row['oa_mean']:.3f}\n")
            txt = str(row["text"])
            buf.write(f"  텍스트: {txt[:100]}"
                      f"{'...' if len(txt) > 100 else ''}\n\n")

    buf.write("── [오분류 fake_cls 분포] ──\n")
    wrong = full_df[~full_df["binary_correct"]]
    if len(wrong) > 0:
        dist = (wrong.groupby("fake_cls").size()
                .reset_index(name="count")
                .sort_values("count", ascending=False))
        buf.write(dist.to_string(index=False))
    buf.write("\n")
    ...
    print(f"\n[CaseStudy] 저장 완료:")
    print(f"  전체 예측 : {prefix}_all_predictions.csv")
    print(f"  신호 요약 : {prefix}_signal_summary.csv")
...
def train_one_run(model, tl, vl, device, args, log=None):
    """
    [FIX]
    - make_optimizer  : 3-group 차등 LR
    - make_scheduler  : warmup + cosine
    - EarlyStopping   : val macro-F1 → val AUC 기준
    - best 모델 저장  : val AUC 기준
    """
    opt      = make_optimizer(model, args)
    sched    = make_scheduler(opt, args)
    # [FIX] AUC 기준 early stopping (불균형 test set에 더 robust)
...
def run_official(dfs: dict, device, tok, prep, args, configure, result_dir):
    """
    DGM4 공식 train/val/test split 사용 (권장 방식).
    val/test 모두 real:fake ≈ 1:2 분포 → AUC 기반 early stopping이 적합.
    """
    ts  = datetime.now().strftime("%Y%m%d_%H%M%S")
    rep = io.StringIO()
    ...
    model = HARFNETdgm4(args.roberta, args.clip_model).to(device)
    if configure:
    ...
    if args.case_study_n > 0:
        ...
        save_case_study_report(cases, full_df, m, "official", result_dir)

    ckpt_path = os.path.join(args.checkpoint_dir,
                              f"harfnet_dgm4_official_{ts}.pt")
    ...
    path = os.path.join(result_dir, f"harfnet_dgm4_official_{ts}.txt")
...
def run_kfold(dfs: dict, device, tok, prep, args, configure, result_dir):
    ...
    train/val fold에 동시에 들어갈 수 있어 소스 누수가 발생 가능.
    성능 비교 목적이면 --kfold 대신 기본 official split을 권장.
    """
    ...
    for fold, (tr_idx, va_idx) in enumerate(
        ...
        ckpt = os.path.join(args.checkpoint_dir,
                            f"harfnet_dgm4_kfold_{ts}_fold{fold}.pt")
    ...
    path = os.path.join(result_dir, f"harfnet_dgm4_kfold_{ts}.txt")
    with open(path, "w", encoding="utf-8") as f:
...
def main():
    pa = argparse.ArgumentParser(
        description="HARFNET-VER3 × DGM4 벤치마크 (수정판 v2)")
    ...
    pa.add_argument("--warmup_epochs", type=int, default=WARMUP_EPOCHS)  # [NEW]
    ...
    # 실험 모드
    # [FIX] 기본값: official split (K-Fold가 아님)
