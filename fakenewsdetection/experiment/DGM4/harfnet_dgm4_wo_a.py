"""
HARFNET-VER3 × DGM4 벤치마크 (AuthorshipModule 제거판)
=========================================================

사용 방법:
  python harfnet_baseline_dgm4.py --data_root /path/to/dgm4_data

  # K-Fold 실행
  python harfnet_baseline_dgm4.py --data_root /path/to/dgm4_data --kfold

  # 주요 옵션
  --epochs 15 --batch_size 16 --lr 2e-4 --warmup_epochs 2
"""
...
LAMBDA_CON        = 0.10   # z_ver 단독 대조 학습
LAMBDA_OA         = 0.20

# 차등 LR 배율
LR_MULT_TEXT   = 0.05   # RoBERTa 상위 레이어
LR_MULT_VISUAL = 0.02   # CLIP layer4 + attnpool
LR_MULT_NEW    = 1.00   # VM / OAM / BI / 헤드
...
FINE_N       = len(FINE_LABELS)   # 4

CLIP_UNFREEZE_KEYS = ("layer4", "attnpool")

LOG_TAG   = "HARFNET-DGM4-noAM"
LOG_BRAND = "HARFNET-DGM4-noAM"
...
def load_dgm4_splits(data_root: str) -> Dict[str, pd.DataFrame]:
    ...
    for split, df in dfs.items():
        ...
        print(f"[{LOG_BRAND}] {split:10s}: {n_total}행  "
...
class DGM4Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return {
            ...
            "binary_label":     torch.tensor(self.binary_lbls[idx],
                                             dtype=torch.long),
            "fine_labels":      torch.tensor(self.fine_lbls[idx],
                                             dtype=torch.float),
            "text":             text,
...
# ─────────────────────────────────────────────────────────────
# 5. 서브모듈
#    [변경] AuthorshipModule 제거
#           GatedPooling / FeedForward는 VM / OAM 에서 계속 사용
# ─────────────────────────────────────────────────────────────
...
class OverAlignModule(nn.Module):
    ...
    def forward(self, V_pat, T_cls, global_sim, p_fake, has_image):
        # [변경] p_auth 인자 제거 → fine_head 입력에서 p_auth 사용 안 함
        B   = V_pat.size(0)
        m1  = has_image.unsqueeze(-1)
        eps = 1e-8
        ...
        F_oa_s = self.oa_scalar_proj(torch.stack(
            [oa_mean, oa_std, oa_uni, oa_max, oa_min, global_sim], dim=-1))
        ...
        # fine_head 입력: [F_oa | p_fake | oa_mean | oa_uni]  (d+3)
        # [변경] 원본의 p_auth 자리를 p_fake로 대체 (시그니처 유지)
        p_fine = self.fine_head(torch.cat([
            F_oa,
            p_fake.unsqueeze(-1),
            oa_mean.unsqueeze(-1),
            oa_uni.clamp(max=50.).unsqueeze(-1)], dim=-1))
        p_fine = p_fine * has_image.unsqueeze(-1)

        return F_oa, p_fine, {
            "oa_mean": oa_mean, "oa_std": oa_std,
            "oa_uniformity": oa_uni, "p_fine": p_fine}


# ── BilinearInteraction: AM 제거에 따른 수정 ────────────────
class BilinearInteraction(nn.Module):
    """
    [변경] auth_low 제거.
    had = ver_low(F_ver) * oa_low(F_oa)   (2-way Hadamard)
    scalars: [p_fake, p_fine_mean]          (2개, p_auth 관련 항 삭제)
    scalar_proj 입력 차원: 6 → 2
    """
    def __init__(self, d, r=BILINEAR_RANK):
        ...
        # [변경] scalar 입력: 6 → 2  (p_fake * has_image, p_fine_mean * has_image)
        self.scalar_proj = nn.Sequential(
            nn.Linear(2, 128), nn.ReLU(), nn.Linear(128, d))

    def forward(self, F_ver, F_oa, p_fake, p_fine_mean, has_image):
        # [변경] F_sty / p_auth 인자 없음
        had = self.ver_low(F_ver) * self.oa_low(F_oa)   # [B, r]
        scalars = torch.stack([
            p_fake       * has_image,
...
# 6. 메인 모델 (AM 제거)
# ─────────────────────────────────────────────────────────────
class HARFNETdgm4_noAM(nn.Module):
    """
    HARFNET-VER3 × DGM4 — AuthorshipModule 제거판.

    [구조]
      Text:   RoBERTa → T_tok, T_cls
      Visual: CLIP RN101 (layer4+attnpool 부분 해동) → V_pat, V_cls
      VM  : VeracityModule(T_tok, V_pat, T_cls, V_cls) → F_ver, p_fake, z_ver
      OAM : OverAlignModule(V_pat, T_cls, gsim, p_fake) → F_oa, p_fine
      BI  : BilinearInteraction(F_ver, F_oa, p_fake, p_fine_mean) → F_bi
      Head: concat[F_ver, F_oa, F_bi]  [B, d*3] → binary logit

    [제거]
      AM (AuthorshipModule): F_sty / p_auth / z_auth 없음
    """
    ...
    def __init__(self, roberta_name=ROBERTA,
        ...
        # CLIP 기본 레이어 Freeze, layer4 + attnpool 만 Unfreeze
    ...
    def train(self, mode=True):
        super().train(mode)
        self.clip_model.eval()   # CLIP BN 안정성 유지
        return self
    ...
    def forward(self, input_ids, attention_mask,
        ...
        # ── OAM: OverAlignModule ─────────────────────────────
        # [변경] p_auth 인자 제거
        F_oa, p_fine, oa_aux = self.om(
            V_pat, T_cls, gsim, p_fake, has_image)

        # ── BI: BilinearInteraction ──────────────────────────
        # [변경] F_sty / p_auth 인자 제거
        p_fine_mean = p_fine.mean(dim=-1)
        F_bi = self.bi(F_ver, F_oa, p_fake, p_fine_mean, has_image)

        # ── Binary Head ──────────────────────────────────────
        # [변경] concat: [F_ver, F_oa, F_bi]  (F_sty 제거)
        feat         = torch.cat([F_ver, F_oa, F_bi], dim=-1)
...
def fine_label_loss(p_fine, fine_labels, has_image):
    ...
    mask_txt[:, :2] = mask[:, :2]   # face_swap, face_attr는 이미지 필요
    return (loss * mask_txt).mean()


def supcon_loss(z, labels, temperature=TEMPERATURE):
    """Supervised Contrastive Loss (z_ver 단독 사용)"""
...
def total_loss(out, binary_label, fine_labels, has_image, args):
    """
    [변경] supcon_loss: z_auth 제거 → z_ver 단독
    4항: binary + fine + con(z_ver only) + oa
    """
    l_bin  = binary_focal_loss(out["binary_logit"], binary_label)
    l_fine = fine_label_loss(out["p_fine"], fine_labels, has_image)
    l_con  = supcon_loss(out["z_ver"], binary_label)        # z_ver 단독
...
# 9. 옵티마이저 & 스케줄러
# ─────────────────────────────────────────────────────────────
def make_optimizer(model: HARFNETdgm4_noAM,
                   args) -> torch.optim.Optimizer:
    """
    3-Group 차등 학습률 옵티마이저
    ─ backbone_text   : RoBERTa 학습 가능 레이어 → lr * LR_MULT_TEXT
    ─ backbone_visual : CLIP layer4/attnpool     → lr * LR_MULT_VISUAL
    ─ new_modules     : VM/OAM/BI/헤드            → lr * LR_MULT_NEW
    """
...
def make_scheduler(optimizer: torch.optim.Optimizer, args):
    """Linear Warmup (warmup_epochs) + Cosine Annealing"""
    warmup = args.warmup_epochs
    total  = args.epochs

    def lr_lambda(ep):   # ep: 0-indexed
        if ep < warmup:
...
# 10. 백본 설정
# ─────────────────────────────────────────────────────────────
def configure_backbones(model: HARFNETdgm4_noAM,
                         text_train_layers: Tuple[int, ...] = (10, 11),
                         unfreeze_clip_top: bool = True):
    """
    RoBERTa: text_train_layers 만 학습 가능
    CLIP    : unfreeze_clip_top=True 면 layer4 + attnpool 학습 가능
    """
...
def run_epoch(model, loader, device, optimizer, train,
    ...
    desc = (f"[{LOG_TAG}] Epoch {epoch_idx}"
            if (train and epoch_idx) else None)
...
# 15. DataLoader 헬퍼
# ─────────────────────────────────────────────────────────────
def _make_dl(df: pd.DataFrame, data_root: str, tok, prep,
             args, shuffle: bool, sampler=None) -> DataLoader:
...
def save_checkpoint(path: str, model, args, metrics=None):
...
# ─────────────────────────────────────────────────────────────
# 16. run_official — DGM4 공식 split (권장)
# ─────────────────────────────────────────────────────────────
def run_official(dfs: dict, device, tok, prep, args,
                 configure: bool, result_dir: str):
    ts  = datetime.now().strftime("%Y%m%d_%H%M%S")
    rep = io.StringIO()
    rep.write(f"###### {LOG_TAG} (Official Split) ######\n\n")
    rep.write(f"[구조] VM + OAM + BI → d*3 → Binary  (AM 없음)\n")
    rep.write(f"[손실] Focal + Fine + ConLoss(z_ver) + OA\n\n")
    ...
    model = HARFNETdgm4_noAM(args.roberta, args.clip_model).to(device)
    if configure:
    ...
    ckpt_path = os.path.join(args.checkpoint_dir,
                              f"harfnet_dgm4_noAM_official_{ts}.pt")
    ...
    path = os.path.join(result_dir, f"harfnet_dgm4_noAM_official_{ts}.txt")
...
def run_kfold(dfs: dict, device, tok, prep, args,
    ...
    all_rep.write(f"[구조] VM + OAM + BI → d*3 → Binary  (AM 없음)\n\n")
    ...
    for fold, (tr_idx, va_idx) in enumerate(
        ...
        ckpt = os.path.join(args.checkpoint_dir,
                             f"harfnet_dgm4_noAM_kfold_{ts}_fold{fold}.pt")
    ...
    g = (f"{LOG_TAG}: BinF1={_mean('bin_f1'):.4f}"
         ...
         f"  Acc={_mean('bin_acc'):.4f}")
    all_rep.write(f"\n\n{'#'*80}\n### SUMMARY\n{g}\n")

    os.makedirs(result_dir, exist_ok=True)
    path = os.path.join(result_dir, f"harfnet_dgm4_noAM_kfold_{ts}.txt")
    with open(path, "w", encoding="utf-8") as f:
...
def main():
    pa = argparse.ArgumentParser(
        description="HARFNET-VER3 × DGM4 — AuthorshipModule 제거판")
    ...
    pa.add_argument("--early_stop_patience",
                    type=int,   default=EARLY_STOP_PATIENCE)
    pa.add_argument("--early_stop_min_delta",
                    type=float, default=EARLY_STOP_MIN_DELTA)

    # 실험 모드 — 기본: official split
    ...
    print(f"[구조] VM + OAM + BI → concat[F_ver, F_oa, F_bi] [d*3] → Binary")
    print(f"[제거] AuthorshipModule (F_sty / p_auth / z_auth 없음)")
    print(f"[손실] Focal({LAMBDA_BINARY_BCE})"
          f" + Fine({LAMBDA_FINE_BCE})"
          f" + Con_zver({LAMBDA_CON})"
          f" + OA({LAMBDA_OA})")
