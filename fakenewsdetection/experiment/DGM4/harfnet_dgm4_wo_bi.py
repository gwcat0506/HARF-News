"""
HARFNET-VER3 × DGM4  —  Ablation: w/o BI (Bilinear Interaction Module)
=========================================================================
"""
...
LOG_TAG   = "HARFNET-DGM4-woBI"
LOG_BRAND = "HARFNET-DGM4-woBI"
...
# ─────────────────────────────────────────────────────────────
# 5. 서브모듈
# [ABLATION] BilinearInteraction 완전 제거
# ─────────────────────────────────────────────────────────────
...
# 6. 메인 모델 — w/o BI
# ─────────────────────────────────────────────────────────────
class HARFNETdgm4_woBI(nn.Module):
    """
    [ABLATION] BilinearInteraction 제거 버전.
    F_bi 를 zero-tensor 로 대체.
    """
...
def run_official(dfs, device, tok, prep, args, configure, result_dir):
    ...
    model = HARFNETdgm4_woBI(args.roberta, args.clip_model).to(device)
    if configure:
    ...
    if args.case_study_n > 0:
        ...
        save_case_study_report(cases, full_df, m, "woBI_official", result_dir)

    ckpt_path = os.path.join(args.checkpoint_dir, f"harfnet_dgm4_woBI_{ts}.pt")
    save_checkpoint(ckpt_path, model, args, m)
    summ = (f"{LOG_TAG}: BinF1={m['bin_f1']:.4f}  AUC={m.get('bin_auc', float('nan')):.4f}")
    print("\n" + "="*70 + "\n" + summ + "\n" + "="*70)
    path = os.path.join(result_dir, f"harfnet_dgm4_woBI_{ts}.txt")
...
def run_kfold(dfs, device, tok, prep, args, configure, result_dir):
    ...
    for fold, (tr_idx, va_idx) in enumerate(skf.split(np.zeros(len(df_tv)), y_tv), 1):
        ...
        ckpt = os.path.join(args.checkpoint_dir,
                            f"harfnet_dgm4_woBI_kfold_{ts}_fold{fold}.pt")
    ...
    all_rep.write(f"\n\n{'#'*80}\n### SUMMARY\n{g}\n")
    path = os.path.join(result_dir, f"harfnet_dgm4_woBI_kfold_{ts}.txt")
    with open(path, "w", encoding="utf-8") as f:
...
def main():
    pa = argparse.ArgumentParser(description="HARFNET-VER3 × DGM4 — Ablation: w/o BI")
