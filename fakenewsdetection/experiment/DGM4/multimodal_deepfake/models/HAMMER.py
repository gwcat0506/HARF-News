...
class HAMMER(nn.Module):
    def __init__(self, 
        ...
        self.binary_only = bool(config.get("binary_only", False))
        self.use_bbox = bool(config.get("use_bbox", True))
        ...
        # creat itm head (binary ITM / real-fake)
        self.itm_head = self.build_mlp(input_dim=text_width, output_dim=2)

        # bbox branch: local cross-attn + aggregator + bbox_head (optional)
        if self.use_bbox:
            ...
            self.bbox_head = None
            self.norm_layer_aggr = None
            self.cls_token_local = None
            self.aggregator = None
            self.norm_layer_it_cross_atten = None
            self.it_cross_attn = None

        # creat multi-cls head (skipped in forward when binary_only)
    ...
    def forward(self, image, label, text, fake_image_box, fake_text_pos, alpha=0, is_train=True):
        if is_train:
            ...
            if self.binary_only:
                _, real_label_pos = get_multi_label(label, image)
                ...
                z = image_embeds.new_tensor(0.0)
                return z, loss_BIC, z, z, z, z
            ...
            if self.binary_only:
                ...
                return logits_real_fake, None, None, None
