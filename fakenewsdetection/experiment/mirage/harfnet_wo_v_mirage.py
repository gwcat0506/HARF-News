"""
HARFNET-VER3-WO-V → MiRAGeNews 이진탐지
==========================================
...
LOG_TAG = "HARFNET-VER3-WO-V-MIRAGE"
...
class OverAlignModule(nn.Module):
    ...
    def forward(self, V_pat, T_cls, global_sim, p_fake, p_auth, has_image):
        ...
        pts=pts.clamp(min=0.)
        oa_mean=pts.mean(1); oa_std=pts.std(1).clamp(0.)
        oa_max=pts.max(1).values; oa_min=pts.min(1).values
...
# ─────────────────────────────────────────────────────────────
# 8. BilinearInteraction2Way (WO-V: auth x oa 2-way)
# ─────────────────────────────────────────────────────────────
class BilinearInteraction2Way(nn.Module):
    ...
    def forward(self, f_sty, f_oa, p_auth, p_af, has_image):
        m        = has_image.unsqueeze(-1)
        hadamard = self.auth_low(f_sty) * self.oa_low(f_oa)
...
# ─────────────────────────────────────────────────────────────
# 9. HARFNETver3_WO_V_Binary
#    WO-V 구조 그대로 + head3(→d*3) → head2(→2) 로 변경
# ─────────────────────────────────────────────────────────────
class HARFNETver3_WO_V_Binary(nn.Module):
    """
    VeracityModule 제거 버전의 MiRAGeNews 이진 탐지 모델.
    WO-V: AM + OAM + BilinearInteraction2Way → head2
    """
    ...
    def forward(self, input_ids, attention_mask, pixel_values,
        ...
        # p_fake: VeracityModule 없으므로 F_sty 기반 probe
        p_fake = self.fake_probe(F_sty).squeeze(-1)

        # global_sim: T_cls ↔ V_cls cosine
        eps   = 1e-8
        t_n   = F.normalize(T_cls, dim=-1, eps=eps)
        v_n   = F.normalize(V_cls, dim=-1, eps=eps)
        gsim  = (t_n * v_n).sum(-1).clamp(-1., 1.) * has_image

        # OAM
        F_oa, p_af, oa_aux = self.om(V_pat, T_cls, gsim, p_fake, p_auth, has_image)

        # BilinearInteraction2Way
        F_bi = self.bi(F_sty, F_oa, p_auth, p_af, has_image)

        # 분류
        logits2 = torch.nan_to_num(
            self.head2(torch.cat([F_sty, F_oa, F_bi], dim=-1)),
    ...
    @classmethod
    def from_wo_v_checkpoint(cls, ckpt_path, device,
                              roberta_name=ROBERTA, clip_name=CLIP_RN101):
        """WO-V 4-class 체크포인트 → head4 제외 인코더 가중치 재사용"""
        model = cls(roberta_name, clip_name).to(device)
        ckpt  = torch.load(ckpt_path, map_location=device)
        state = ckpt.get("model_state_dict", ckpt)
        filtered = {k: v for k, v in state.items()
                    if not k.startswith("head4.")}
        missing, unexpected = model.load_state_dict(filtered, strict=False)
        print(f"[{LOG_TAG}] WO-V 체크포인트 로드: "
...
def attribute_bce_loss(p, pos_mask, has_image=None):
    ...
    loss  = F.binary_cross_entropy(p.clamp(1e-6,1-1e-6), label, reduction="none")
...
def total_loss(out, y, has_image, args):
    """
    WO-V 활성 손실: Focal + OA_Loss + BCE(Auth) + AF_Cond_BCE
    fake_BCE / SupCon 없음 (VeracityModule 제거)
    """
...
def _monitor(model, loader, device):
    ...
    try:
        with torch.no_grad():
            bufs={k:[] for k in ["lbl","pa","paf","oam","gs"]}
            ...
            paf=torch.cat(bufs["paf"]); oam=torch.cat(bufs["oam"])
...
def run_official_split(args):
    ...
    # 모델 생성
    if args.wo_v_ckpt:
        model = HARFNETver3_WO_V_Binary.from_wo_v_checkpoint(
            args.wo_v_ckpt, DEVICE, args.roberta, args.clip_model)
    else:
        model = HARFNETver3_WO_V_Binary(args.roberta, args.clip_model).to(DEVICE)
    ...
    plot_path = os.path.join(RESULT_DIR, f"harfnet_wo_v_mirage_curve_{ts}.png")
    ...
    save_checkpoint(
        os.path.join(CHECKPOINT_DIR, f"harfnet_wo_v_mirage_{ts}.pt"),
        model, args, all_metrics)
    path = os.path.join(RESULT_DIR, f"harfnet_wo_v_mirage_{ts}.txt")
...
def run_kfold_split(args):
    ...
    for fold_idx, (tr_idx, va_idx) in enumerate(
        ...
        tr_ds = MiRAGeNewsDataset(base_train, tok, prep, args.max_length,
                                  indices=tr_idx.tolist())
        va_ds = MiRAGeNewsDataset(base_train, tok, prep, args.max_length,
        ...
        plot_path = os.path.join(
            RESULT_DIR, f"harfnet_wo_v_mirage_curve_fold{fold_idx}_{ts}.png")
    ...
    path = os.path.join(RESULT_DIR, f"harfnet_wo_v_mirage_kfold{args.kfold_splits}_{ts}.txt")
    with open(path, "w", encoding="utf-8") as f: f.write(rep.getvalue())
    print(f"\n리포트: {path}")
...
def main():
    pa = argparse.ArgumentParser(
        description="HARFNET-VER3-WO-V → MiRAGeNews 이진탐지")

    pa.add_argument("--hf_dataset_id",  default=HF_DATASET_ID)
    pa.add_argument("--checkpoint_dir", default=CHECKPOINT_DIR)
    pa.add_argument("--wo_v_ckpt",      default=None,
                    help="WO-V 4-class 체크포인트 (인코더 재활용, 선택)")
