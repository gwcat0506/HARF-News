"""
HARFNET-VER3 → MiRAGeNews 이진탐지  [Ablation: w/o BM + w/o OAM + w/o AM]
=============================================================================
"""
...
LOG_TAG   = "HARFNET-woBM-OAM-A-MIRAGE"
LOG_BRAND = "HARFNET-woBM-OAM-A-MIRAGE"
...
class MiRAGeNewsDataset(Dataset):
    def __init__(self, hf_split, tokenizer, clip_preprocess,
        ...
        for cand in ("caption", "text", "headline", "final_headline"):
            ...
            raise ValueError(f"텍스트 컬럼 없음: {cols}")
        for cand in ("image", "img"):
            if cand in cols:
                self._img_col = cand
                break
        else:
            raise ValueError(f"이미지 컬럼 없음: {cols}")
        for cand in ("label", "labels", "fake"):
            if cand in cols:
                self._lbl_col = cand
                break
        else:
            raise ValueError(f"레이블 컬럼 없음: {cols}")
...
# ─────────────────────────────────────────────────────────────
# 5. 메인 모델 — w/o BM + w/o OAM + w/o AM (DGM4 zero-pad 방식)
# ─────────────────────────────────────────────────────────────
class HARFNETver3_woBMOAM_A(nn.Module):
    """
    AM / OA / BM 제거. VM만 활성.
    """
    ...
    def forward(self, input_ids, attention_mask, pixel_values,
        ...
        # [ABLATION] AM / OA / BM 없음 → zero-tensor (DGM4 wo_bi_oam 패턴)
        F_sty = torch.zeros(B, self._d, device=device, dtype=F_ver.dtype)
        F_oa  = torch.zeros(B, self._d, device=device, dtype=F_ver.dtype)
        F_bi  = torch.zeros(B, self._d, device=device, dtype=F_ver.dtype)
        p_auth = torch.zeros(B, device=device, dtype=p_fake.dtype)
        p_af   = torch.zeros(B, device=device, dtype=p_fake.dtype)
    ...
    try:
        with torch.no_grad():
            ...
            for ci, cn in enumerate(BINARY_CLASS_NAMES):
                mask = (lbl == ci)
                if mask.sum() > 0:
                    parts.append(f"{cn}:p_fake={pf[mask].mean():.2f}")
...
def plot_epoch_curves(epoch_log, save_path, title=""):
    ...
    ax1.plot(epochs, [e["train_loss"] for e in epoch_log],
             "o-", color="#2563EB", lw=2, ms=5, label="Train")
    ax1.plot(epochs, [e["val_loss"] for e in epoch_log],
             "s--", color="#DC2626", lw=2, ms=5, label="Val")
...
def load_hf_splits(hf_id=HF_DATASET_ID):
    ...
    print(f"[{LOG_TAG}] 스플릿: {list(ds.keys())}")
    return ds
...
def resolve_test_splits(ds):
    ...
    print(f"[{LOG_TAG}] 테스트 스플릿: {result}")
    return result
...
def run_official_split(args):
    ...
    def _wrap(s):
        return MiRAGeNewsDataset(hf_ds[s], tok, prep, args.max_length)
    ...
    model = (HARFNETver3_woBMOAM_A.from_harf_checkpoint(
    ...
    rep.write("[제거] BM + OAM + AM  [유지] VM\n")
    rep.write("[head] d×4 (zero-pad) → 512 → 2\n")
    ...
    plot_epoch_curves(
        epoch_log,
        os.path.join(RESULT_DIR, f"wobmoam_a_mirage_curve_{ts}.png"),
    ...
    save_checkpoint(
        os.path.join(CHECKPOINT_DIR, f"wobmoam_a_mirage_single_{ts}.pt"),
        model, args, all_metrics)
    path = os.path.join(RESULT_DIR, f"wobmoam_a_mirage_single_{ts}.txt")
...
def run_kfold_split(args):
    ...
    for fold_idx, (tr_idx, va_idx) in enumerate(
        ...
        plot_epoch_curves(
            epoch_log,
            os.path.join(RESULT_DIR, f"wobmoam_a_mirage_curve_fold{fold_idx}_{ts}.png"),
    ...
    for sname in test_splits:
        global_metrics[sname] = {
            k: float(np.mean([fr[sname][k] for fr in fold_results]))
            for k in ("acc", "f1", "f1_ai", "auc")
        }
    for sname in test_splits:
    ...
    torch.save({
        ...
        "saved_at": datetime.now().isoformat(timespec="seconds"),
    }, os.path.join(CHECKPOINT_DIR, f"wobmoam_a_mirage_kfold{args.kfold_splits}_{ts}.pt"))

    path = os.path.join(
        RESULT_DIR, f"wobmoam_a_mirage_kfold{args.kfold_splits}_{ts}.txt")
    with open(path, "w", encoding="utf-8") as f:
        f.write(rep.getvalue())
    print(f"\n리포트: {path}")
...
def main():
    pa = argparse.ArgumentParser(
        description="HARFNET w/o BM w/o OAM w/o AM (VM only, DGM4 zero-pad head)")
    ...
    print("[Ablation] w/o Bilinear  w/o OverAlign  w/o Authorship")
    print("[유지]     VeracityModule only")
    print("[head]     d×4 zero-pad → 512 → 2  (DGM4 wo_bi_oam 패턴 + AM 제거)")
