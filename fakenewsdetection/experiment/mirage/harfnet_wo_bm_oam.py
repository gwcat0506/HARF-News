"""
HARFNET-VER3 → MiRAGeNews 이진탐지  [Ablation: w/o OA + w/o BM]
==================================================================
"""
...
LAMBDA_FAKE_BCE = 0.30   # BCE(p_fake)
LAMBDA_AUTH_BCE = 0.15   # BCE(p_auth)
# LAMBDA_OA     제거 (OverAlignModule 없음)
# LAMBDA_AF_BCE 제거 (p_af 없음)
...
LOG_TAG   = "HARFNET-noOA-noBM-MIRAGE"
LOG_BRAND = "HARFNET-noOA-noBM-MIRAGE"
...
# 6. 메인 모델 — OA·BM 제거 버전
#
#    CLIP visual → V_pat(patch) + V_cls(global)
#    AM(T_tok)        → F_sty, p_auth
#    VM(T_tok, V_pat) → F_ver, p_fake
#    OA(V_pat, T_cls) → F_oa,  p_af      ← 제거
#    BM(F_sty, F_ver, F_oa, ...) → F_bi  ← 제거
#    head2([F_sty, F_ver, F_oa, F_bi])   d×4 → 2
#
#  [본 파일]
#    AM(T_tok)        → F_sty, p_auth
#    VM(T_tok, V_pat) → F_ver, p_fake
#    head2([F_sty, F_ver])               d×2 → 2
# ─────────────────────────────────────────────────────────────
class HARFNETver3_noOA_noBM(nn.Module):
    def __init__(self, roberta_name=ROBERTA, clip_name=CLIP_RN101,
        ...
        self.deep_proj = nn.Linear(2048, d)   # patch feature 투영
        self.cls_proj  = nn.Linear(clip_edim, d)   # global feature 투영

        # ── AM / VM (OA·BM 제거) ──
        self.am = AuthorshipModule(d, n_heads, dropout)
        self.vm = VeracityModule(d, n_heads, dropout)

        # ── head2: d×2 입력 (원본 d×4 → d×2로 축소) ──
    ...
    def _clip_visual_forward(self, pix, has_image):
        ...
        x = vis.layer3(x); deep = vis.layer4(x)
        v_global = vis.attnpool(deep)
        # patch feature: [B, H*W, 2048] → 투영 후 [B, H*W, d]
        V_pat = self.deep_proj(
            deep.flatten(2).transpose(1, 2)) * has_image.view(B, 1, 1)
        # global feature: [B, d]
    ...
    def forward(self, input_ids, attention_mask, pixel_values,
        ...
        # ── 분류 헤드 (OA·BM 없이 F_sty + F_ver만 결합) ──
        fused  = torch.cat([F_sty, F_ver], dim=-1)   # [B, d×2]
    ...
    def from_harf_checkpoint(cls, ckpt_path, device,
        ...
        """
        HARFNET 4-class VER3 체크포인트 → head2/om/bi 제외, 인코더+AM+VM만 재사용.
        """
        model  = cls(roberta_name, clip_name).to(device)
        ckpt   = torch.load(ckpt_path, map_location=device)
        state  = ckpt.get("model_state_dict", ckpt)
        # head4/head2(출력 불일치), om(OverAlign), bi(Bilinear) 제외
        exclude_prefixes = ("head4.", "head2.", "om.", "bi.")
...
def binary_focal_loss(logits2, targets, gamma=2.0):
    ...
    ce  = F.cross_entropy(logits2, targets,
                          weight=inv.to(logits2.device), reduction="none")
...
def total_loss(out, y, has_image, args):
    """
    Focal + λ_fake·BCE(p_fake) + λ_auth·BCE(p_auth)
    OA 손실(λ_oa·overalign) 및 AF-BCE(λ_af·p_af) 완전 제거.
    """
...
def _monitor(model, loader, device):
    """학습 중 p_auth, p_fake 분포 간이 모니터링 (OA 항목 제거)"""
    model.eval()
    try:
        with torch.no_grad():
            ...
            for ci, cn in enumerate(BINARY_CLASS_NAMES):
                ...
                if mask.sum() > 0:
                    parts.append(
                        f"{cn}: p_auth={pauth[mask].mean():.2f}"
                        f"  p_fake={pf[mask].mean():.2f}")
...
def run_epoch(model, loader, device, optimizer, train,
    ...
    return {
        ...
        "f1_real":  float(f1_score(ys, ps, pos_label=0,
                                   average="binary", zero_division=0)),
    }
...
# 9. 학습 헬퍼
# ─────────────────────────────────────────────────────────────
def freeze_backbones(model: HARFNETver3_noOA_noBM):
...
def plot_epoch_curves(epoch_log: list, save_path: str, title: str = ""):
    ...
    ax2.plot(epochs, val_f1, "^-", color="#16A34A",
...
def run_official_split(args):
    ...
    if args.harf_ckpt:
        model = HARFNETver3_noOA_noBM.from_harf_checkpoint(
    ...
    rep.write(f"[제거] OverAlignModule(OA) + BilinearInteraction(BM)\n")
    rep.write(f"[유지] AuthorshipModule(AM) + VeracityModule(VM)\n")
    rep.write(f"[head] d×2 → 512 → 2\n")
    ...
    plot_path = os.path.join(RESULT_DIR, f"nooa_nobm_mirage_curve_{ts}.png")
    ...
    save_checkpoint(
        os.path.join(CHECKPOINT_DIR, f"nooa_nobm_mirage_single_{ts}.pt"),
        model, args, all_metrics)

    path = os.path.join(RESULT_DIR, f"nooa_nobm_mirage_single_{ts}.txt")
...
def run_kfold_split(args):
    ...
    for fold_idx, (tr_idx, va_idx) in enumerate(
        ...
        plot_path = os.path.join(
            RESULT_DIR, f"nooa_nobm_mirage_curve_fold{fold_idx}_{ts}.png")
        plot_epoch_curves(epoch_log, plot_path,
        ...
        summ = "  ".join(
            f"{s}: F1={fold_metrics[s]['f1']:.4f}" for s in test_splits)
    ...
    torch.save({
        ...
        "saved_at":       datetime.now().isoformat(timespec="seconds"),
                    ...
                    f"nooa_nobm_mirage_kfold{args.kfold_splits}_{ts}.pt"))

    path = os.path.join(
        RESULT_DIR, f"nooa_nobm_mirage_kfold{args.kfold_splits}_{ts}.txt")
    with open(path, "w", encoding="utf-8") as f: f.write(rep.getvalue())
    print(f"\n리포트: {path}")
...
def main():
    pa = argparse.ArgumentParser(
        description="HARFNET-VER3 w/o OA w/o BM → MiRAGeNews 이진탐지")
    ...
    pa.add_argument("--harf_ckpt",     default=None,
                    help="VER3 체크포인트 경로 (AM+VM 인코더 재사용, 선택)")
    ...
    print(f"[Ablation] OverAlignModule(OA) 제거 + BilinearInteraction(BM) 제거")
    print(f"[유지]     AuthorshipModule(AM) + VeracityModule(VM)")
    print(f"[head]     d×2 → 512 → 2  (원본 d×4 → 512 → 2)")
    print(f"[손실]     Focal"
          f" + {args.lambda_fake_bce}·BCE(fake)"
          f" + {args.lambda_auth_bce}·BCE(auth)")
    print(f"           (OA loss / p_af BCE 제거)")
